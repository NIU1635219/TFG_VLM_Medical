...
*   **Aversión al Área Cero (Fallback Global):** Se descubrió una limitación crítica cuando el modelo no detecta ninguna lesión pero el esquema estricto le obliga a generar coordenadas. En lugar de devolver coordenadas nulas (`[0, 0, 0, 0]`), el modelo invariablemente devuelve `[0, 0, 1000, 1000]`, englobando la imagen entera. Este comportamiento de *Fallback Global* es consecuencia de las funciones de pérdida (ej. GIoU) utilizadas durante su pre-entrenamiento, las cuales penalizan matemáticamente las cajas con área cero. Ante el conflicto entre la instrucción del *prompt* y el sesgo geométrico de sus pesos pre-entrenados, el modelo opta por la solución matemáticamente "segura", sacrificando la exactitud lógica.
*   **Desacoplamiento Arquitectónico (Late vs. Early Fusion):** Se ha observado que los modelos cuyas modalidades de visión y texto no han sido entrenadas conjuntamente desde las capas más profundas (*Late Fusion*, ej. MiniCPM-o) sufren una severa "agnosia espacial". Comprenden la imagen general, pero fracasan al situar los elementos físicamente en el lienzo. Por este motivo, el proyecto se centró casi en exclusiva en la familia **Qwen3.5** (*Early Fusion* o Fusión Temprana nativa). Sin embargo, se demostró que, aunque Qwen3.5 reduce drásticamente la agnosia espacial, ni siquiera el entrenamiento en la misma capa logra superar los problemas subyacentes de la *patchification* descritos anteriormente.
...
*   **Exceso de Libertad (`max_tokens` elevado):** Al configurar un límite de tokens amplio en la API, se le otorga al modelo "demasiado espacio vacío" que, probabilísticamente, siente la necesidad de rellenar. 
