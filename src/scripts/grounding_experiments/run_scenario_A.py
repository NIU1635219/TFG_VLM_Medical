"""Escenario A: inferencia zero-shot para grounding + diagnóstico de pólipos.

Este script ejecuta un batch sobre el CSV de ground truth y guarda resultados
incrementales en JSONL para análisis posterior.
"""
...
SCENARIO_A_PROMPT_TEMPLATE = (
    "Eres un endoscopista experto. Localiza la única lesión en esta imagen devolviendo "
    "sus coordenadas exactas en una escala normalizada [0-1000]. A continuación, "
    "describe detalladamente su morfología, bordes y patrón vascular, y emite un "
    "diagnóstico histológico (AD, HP o ASS). "
)
...
def build_parser() -> argparse.ArgumentParser:
    """Construye el parser CLI para Escenario A."""
    parser = argparse.ArgumentParser(
        description="Run Scenario A (zero-shot grounding + diagnosis) over ground truth CSV."
    )
    parser.add_argument(
        "--model",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "data/processed/scenario_results/scenario_A/run_*/results.jsonl"
        ),
    )
    parser.add_argument(
        "--limit",
...
def run(args: argparse.Namespace, reporter: Reporter | None = None) -> int:
    """Ejecuta el batch del Escenario A y guarda resultados incrementales en JSONL."""
    started_at = time.perf_counter()
    input_csv = str(args.input_csv)
    output_path = resolve_output_jsonl_path(
        raw_output=args.output,
        default_output=default_scenario_output_path(scenario_name="scenario_A"),
    )
        ...
        for index, row in enumerate(row_iterable, start=1):
            ...
            try:
                parsed_response, telemetry_payload = safe_inference_with_optional_telemetry(
                    loader=loader,
                    image_path=image_path,
                    prompt=SCENARIO_A_PROMPT_TEMPLATE,
            ...
            annotated_path = write_comparison_image(
                image_path=image_path,
