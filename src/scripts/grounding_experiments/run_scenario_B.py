"""Escenario B: inferencia asistida por clase real para grounding + diagnóstico.

Este script ejecuta un batch sobre el CSV de ground truth e inyecta la clase
real del pólipo en el prompt de entrada para guiar la localización.
"""
...
def build_parser() -> argparse.ArgumentParser:
    """Construye el parser CLI para Escenario B."""
    parser = argparse.ArgumentParser(
        description="Run Scenario B (class-assisted grounding + diagnosis) over ground truth CSV."
    )
    parser.add_argument(
        "--model",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "data/processed/scenario_results/scenario_B/run_*/results.jsonl"
        ),
    )
    parser.add_argument(
        "--limit",
...
def run(args: argparse.Namespace, reporter: Reporter | None = None) -> int:
    """Ejecuta el batch del Escenario B y guarda resultados incrementales en JSONL."""
    started_at = time.perf_counter()
    output_path = resolve_output_jsonl_path(
        raw_output=args.output,
        default_output=default_scenario_output_path(scenario_name="scenario_B"),
    )
    ...
    markdown_path = generate_single_detection_markdown_report(
        run_dir=run_dir,
        report_title="Scenario B Report",
        scenario_name="scenario_B",
        model_id=str(args.model),
