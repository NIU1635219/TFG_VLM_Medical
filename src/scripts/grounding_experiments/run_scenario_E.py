...
def build_parser() -> argparse.ArgumentParser:
    """Construye el parser CLI para Escenario E."""
    parser = argparse.ArgumentParser(
        description="Run Scenario E (quality ceiling: ROI crop + class injection) over ground truth CSV."
    )
    parser.add_argument(
        "--model",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "data/processed/scenario_results/scenario_E/run_*/results.jsonl"
        ),
    )
    parser.add_argument(
        "--limit",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Usa la misma seed en escenarios A/B/C/D/E para evaluar el mismo subconjunto."
        ),
    )
    parser.add_argument(
        "--no-progress",
...
def run(args: argparse.Namespace, reporter: Reporter | None = None) -> int:
    """Ejecuta el batch del Escenario E y guarda resultados incrementales en JSONL."""
    ...
    try:
        ...
        for index, row in enumerate(row_iterable, start=1):
            ...
            try:
                bbox_forced_image_buffer = draw_bbox_and_save_temp_image(
                    image_path=image_path,
                ...
                parsed_response, telemetry_payload = safe_inference_with_optional_telemetry(
                    loader=loader,
                    image_path=bbox_forced_image_buffer,
    ...
    markdown_path = generate_single_detection_markdown_report(
        run_dir=run_dir,
        report_title="Scenario E Report",
        scenario_name="scenario_E",
        model_id=str(args.model),
