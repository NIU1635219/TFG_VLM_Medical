"""Escenario F: reporte clínico asistido con clase GT inyectada y bbox predicha.

Este script ejecuta un batch sobre el CSV de ground truth e inyecta la clase
histológica real en el prompt. El modelo debe localizar la lesión, redactar una
explicación clínica de alta calidad y cerrar con la etiqueta final.
"""
...
SCENARIO_F_PROMPT_TEMPLATE = (
    "El análisis anatomopatológico ya confirmó que la lesión de esta colonoscopia es de tipo {gt_cls}. "
    "Tu tarea NO es diagnosticar ni proponer tratamiento. "
    "Debes: 1) localizar la lesión devolviendo xmin, ymin, xmax, ymax en escala normalizada [0-1000], "
    "2) redactar un informe clínico explicativo de alto nivel justificando visualmente ese diagnóstico con "
    "terminología estandarizada (Paris, NICE/Kudo, WASP) y 3) cerrar con final_diagnosis_class={gt_cls}."
)


def build_parser() -> argparse.ArgumentParser:
    """Construye el parser CLI para Escenario F."""
    parser = argparse.ArgumentParser(
        description="Run Scenario F (assisted clinical report + grounding) over ground truth CSV."
    )
    parser.add_argument(
        "--model",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "data/processed/scenario_results/scenario_F/run_*/results.jsonl"
        ),
    )
    parser.add_argument(
        "--limit",
...
def run(args: argparse.Namespace, reporter: Reporter | None = None) -> int:
    """Ejecuta el batch del Escenario F y guarda resultados incrementales en JSONL."""
    started_at = time.perf_counter()
    output_path = resolve_output_jsonl_path(
        raw_output=args.output,
        default_output=default_scenario_output_path(scenario_name="scenario_F"),
    )
        ...
        for row in rows:
            ...
            skeleton_records.append(
                build_scenario_record(
                    scenario_name="scenario_F",
                    model_id=str(args.model),
                    schema_name="AssistedClinicalReport",
    ...
    try:
        ...
        for index, row in enumerate(row_iterable, start=1):
            ...
            try:
                ...
                processed_fail += 1
                processed_skip += 1
                print(f"[WARN] Saltando fila por datos inválidos: {error}")
                upsert_scenario_result_record(
                    output_path=output_path,
            ...
            try:
                parsed_response, telemetry_payload = safe_inference_with_optional_telemetry(
                    image_path=image_path,
                    loader=loader,
                    prompt=SCENARIO_F_PROMPT_TEMPLATE.format(gt_cls=gt_cls),
                    schema=AssistedClinicalReport,
                )
            except Exception as error:
                processed_fail += 1
                print(
                    "[WARN] Inference/Pydantic falló para "
    ...
    markdown_path = generate_single_detection_markdown_report(
        run_dir=run_dir,
        report_title="Scenario F Report",
        scenario_name="scenario_F",
        model_id=str(args.model),
